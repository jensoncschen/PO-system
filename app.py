...
# --- 頁面設定 ---
st.set_page_config(page_title="雲端訂購系統 (條碼絕對命中版)", layout="wide", page_icon="🛍️")
...
# ★★★ 新增：專門對付 Google Sheets 奇怪格式的條碼清洗器 ★★★
def clean_barcode(val):
    s = str(val).strip() # 去除前後空白
    if s.endswith('.0'): # 去除浮點數的小數點
        s = s[:-2]
...
if page == "🛒 前台：下單作業":
    ...
    # --- 📷 條碼快搜區 ---
    st.markdown("#### 📷 步驟一：條碼快搜 (優先)")
    barcode_input = st.text_input(
        "游標停在此處，刷入條碼或輸入後按 Enter", 
        placeholder="例如輸入 12345 (若使用條碼，將無視下方品牌/品類篩選)...",
    ...
    # ★★★ 關鍵邏輯：決定選單要顯示什麼商品 ★★★
    if barcode_input:
        # 如果有條碼，無視前面的品牌和品類！直接在全資料庫找條碼！
        clean_input = barcode_input.strip()
        df_step2 = df_products[df_products["國際條碼"] == clean_input]
        
        if df_step2.empty:
            st.error(f"❌ 資料庫中找不到條碼為「{clean_input}」的商品！")
    else:
        # 沒有條碼，就乖乖按照品牌、品類篩選
    ...
    # ★ 如果是條碼搜尋，我們直接幫他「自動全選」！
    default_selections = display_options if barcode_input and display_options else []

    selected_displays = st.multiselect(
        "3️⃣ 選擇商品 (可手動點選，最多20樣)", 
        options=display_options, 
        default=default_selections, # 自動填入條碼找到的商品
        max_selections=20,
